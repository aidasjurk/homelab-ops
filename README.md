...
| **SSH Hardening** | Security | ✅ Working | Ed25519 keys only, password auth disabled ([configs/99-hardened.conf](configs/99-hardened.conf)), root login denied, verified via `sshd -T` |
...
| **Local Snapshot Script** | Backups | ✅ Working | [scripts/backup-stacks.sh](scripts/backup-stacks.sh) active; creates atomic local tarballs with 7-day retention & offsite sync |
...
- **Dynamic User Environment Portability:** Dynamically resolves `USER_HOME` via `${SUDO_USER:-$USER}` to export `RCLONE_CONFIG="${USER_HOME}/.config/rclone/rclone.conf"`, avoiding hardcoded user paths and preventing runtime failures when executed under sudo or headless systemd timers.
...
| *2026-09-18* | [scripts/backup-stacks.sh](scripts/backup-stacks.sh) failed with `rclone.conf not found in /root/.config` | Script executed under `sudo`/root, but `rclone config` was created in the invoking user's home directory. | Added dynamic `USER_HOME` detection (`${SUDO_USER:-$USER}`) in [scripts/backup-stacks.sh](scripts/backup-stacks.sh) to resolve `rclone.conf` portably. |
| *2026-09-18* | `sshd -T` reported `passwordauthentication yes` during security audit | `/etc/ssh/sshd_config` had `#PasswordAuthentication yes` commented out, defaulting to Debian default `yes`. | Created [configs/99-hardened.conf](configs/99-hardened.conf) with explicit `PasswordAuthentication no` and `PermitRootLogin no`, tested via `sshd -t`, and reloaded daemon. |
